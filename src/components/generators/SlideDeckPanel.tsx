...
export default function SlideDeckPanel({ onBack }: SlideDeckPanelProps) {
  ...
  return (
    <div className="slide-deck-panel flex h-full w-full flex-col overflow-hidden bg-zinc-950/70 select-none">
      {/* ─── Sticky Header ─── */}
      <div className="panel-header sticky top-0 z-20 flex flex-col gap-2 border-b border-zinc-800/80 bg-zinc-950/95 px-3 py-2.5 backdrop-blur-xl shrink-0">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1 rounded-lg border border-cyan-500/40 bg-cyan-950/40 px-2.5 py-1 text-xs font-medium text-cyan-300 transition hover:bg-cyan-900/50 active:scale-95"
        ...
        <div className="flex items-center gap-2 pt-0.5">
          <div className="flex h-6 w-6 items-center justify-center rounded-md border border-cyan-500/30 bg-cyan-950/50 text-cyan-400 shrink-0">
            <Presentation className="h-3.5 w-3.5" />
          </div>
          <div className="min-w-0 flex-1">
            <h3 className="text-xs font-semibold text-zinc-100 truncate">Executive Slide Deck Generator</h3>
            <p className="text-[10px] text-zinc-500 truncate">5-slide presentation briefing</p>
          </div>
        </div>
      </div>

      {/* ─── Scrollable Slide Content Area (Strictly Constrained) ─── */}
      <div className="panel-content flex-1 overflow-y-auto p-3 scroll-smooth space-y-3">
        {/* Slide Card Container */}
        <div className="rounded-xl border border-zinc-800/90 bg-gradient-to-b from-zinc-900/90 via-zinc-950 to-zinc-900/60 p-3.5 sm:p-4 shadow-xl">
          ...
          <div className="flex items-center justify-between border-b border-zinc-800/60 pb-2">
            ...
            <span className="rounded bg-cyan-950/80 px-2 py-0.5 border border-cyan-500/30 text-[9px] font-mono uppercase text-cyan-300">
              Executive Briefing
            </span>
          </div>

          {/* Slide Title */}
          <h4 className="mt-2.5 text-sm sm:text-base font-bold text-zinc-100 leading-snug">
            {slide.title}
          </h4>
          ...
          {/* Key Takeaway Box */}
          <div className="mt-3.5 rounded-lg border border-cyan-500/20 bg-cyan-950/20 p-2.5">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-cyan-300 flex items-center gap-1">
              <Sparkles className="h-3 w-3 shrink-0 text-cyan-400" />
              Key Takeaway
            </p>
            <p className="mt-0.5 text-[11px] text-zinc-300 leading-relaxed">
              {slide.takeaway}
            </p>
          </div>
        </div>
      ...
      <div className="sticky bottom-0 z-20 flex items-center justify-between border-t border-zinc-800/80 bg-zinc-950/95 p-2.5 backdrop-blur-xl shrink-0">
        {/* Dot Indicators */}
          ...
          {slides.map((_, idx) => (
            <button
              ...
              className={cn(
                ...
                currentSlide === idx ? "w-5 bg-cyan-400 shadow-[0_0_6px_rgba(6,182,212,0.8)]" : "w-1.5 bg-zinc-700 hover:bg-zinc-500"
              )}
            />
          ))}
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="flex items-center gap-1 rounded-lg border border-zinc-700 bg-zinc-900 px-2.5 py-1 text-xs font-medium text-zinc-300 disabled:opacity-30 hover:bg-zinc-800 active:scale-95 transition"
          >
          ...
          <button
            ...
            className="flex items-center gap-1 rounded-lg border border-cyan-500/40 bg-cyan-950/50 px-2.5 py-1 text-xs font-medium text-cyan-300 disabled:opacity-30 hover:bg-cyan-900/60 active:scale-95 transition"
          >
            <span>Next</span>
