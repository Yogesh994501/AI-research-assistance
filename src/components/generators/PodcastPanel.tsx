  ...
  return (
    <div className="slide-deck slide-deck-panel flex h-full w-full flex-col overflow-hidden select-none">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <select
              ...
              className="rounded-lg border border-white/[0.15] bg-white/[0.08] px-2 py-1 text-xs text-white focus:outline-none"
            >
              <option value="0.85" className="bg-zinc-900">0.85x</option>
              <option value="1" className="bg-zinc-900">1.0x</option>
              <option value="1.15" className="bg-zinc-900">1.15x</option>
              <option value="1.3" className="bg-zinc-900">1.3x</option>
            </select>
          </div>
        </div>

        <div className="flex items-center gap-2 pt-0.5">
          <div className="flex h-6 w-6 items-center justify-center rounded-lg border border-emerald-400/30 bg-emerald-400/10 text-emerald-300 shrink-0">
            <Headphones className="h-3.5 w-3.5" />
          </div>
          <div className="min-w-0 flex-1">
            <h3 className="text-xs font-bold text-white truncate">Audio Podcast Narrator</h3>
            <p className="text-[10px] text-[#94A3B8] truncate">2-minute conversational research briefing</p>
          </div>
        </div>
      </div>
      ...
      <div className="panel-content flex-1 overflow-y-auto p-3.5 scroll-smooth space-y-3">
        ...
        <div className="slide-card flex items-center justify-between p-3.5 shadow-lg">
          <div className="flex items-center gap-3 min-w-0">
            <button
              onClick={toggleSpeech}
              className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-400 text-zinc-950 shadow-[0_0_16px_rgba(52,211,153,0.4)] transition hover:bg-emerald-300 hover:scale-105 active:scale-95 shrink-0"
            >
              {isPlaying ? <Pause className="h-4 w-4 shrink-0 fill-zinc-950" /> : <Play className="h-4 w-4 ml-0.5 shrink-0 fill-zinc-950" />}
            </button>
            <div className="min-w-0">
              <p className="text-xs font-bold text-white truncate">
                {isPlaying ? "Playing Briefing..." : "Ready to Listen"}
              </p>
              <p className="text-[10px] text-[#CBD5E1] flex items-center gap-1 mt-0.5 truncate">
          ...
          <button
            onClick={handleStop}
            className="btn-secondary rounded-lg p-2 text-[#CBD5E1] active:scale-95 shrink-0"
            title="Reset Audio"
          >
          ...
          {podcastScript.map((line, idx) => (
            <div
              ...
              className={`rounded-xl border p-3 text-xs leading-relaxed ${
                idx % 2 === 0
                  ? "border-emerald-400/20 bg-emerald-400/[0.05] text-[#E2E8F0]"
                  : "border-cyan-400/20 bg-cyan-400/[0.05] text-[#E2E8F0]"
              }`}
            >
              <div className="mb-1.5 flex items-center gap-2">
                <span className={`font-bold text-xs ${idx % 2 === 0 ? "text-emerald-300" : "text-cyan-300"}`}>
                  {line.speaker}
                </span>
                <span className="rounded-md bg-white/[0.08] px-1.5 py-0.5 text-[9px] text-[#CBD5E1] border border-white/[0.08]">
                  {line.role}
                </span>
              </div>
              <p className="text-[11px] leading-relaxed text-[#CBD5E1]">{line.text}</p>
            </div>
          ))}
        </div>
      </div>
